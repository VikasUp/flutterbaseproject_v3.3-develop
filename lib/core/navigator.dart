...
class AppNavigator {
  ...
  Future<void> handleNotificationRedirection(
    ...
    var type = Utilities.enumFromString(
        payload?.type ?? '', NotificationRedirection.values);

    dynamic route;
    switch (type) {
      case NotificationRedirection.loginPassword:
        route = const LoginPage();
        break;
      case NotificationRedirection.registerPassword:
        route = const RegistrationPage();
        break;
      case NotificationRedirection.registerOTP:
        route = const RegisterWithOTPPage();
        break;
    }
    if (route != null) {
